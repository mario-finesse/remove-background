...
def im_preprocess(im,size):
    ...
    if(len(size)<2):
        ...
        im_tensor = F.interpolate(im_tensor, size=size, mode="bilinear", align_corners=True)
...
def gt_preprocess(gt,size):
    ...
    if(len(size)<2):
        ...
        gt_tensor = F.interpolate(gt_tensor, size=size, mode="bilinear", align_corners=True)
...
class GOSResize(object):
    ...
    def __call__(self,sample):
        ...
        image = torch.squeeze(F.interpolate(torch.unsqueeze(image,0), size=self.size, mode='bilinear', align_corners=True),dim=0)

        label = torch.squeeze(F.interpolate(torch.unsqueeze(label,0), size=self.size, mode='bilinear', align_corners=True),dim=0)
